...
static void cmd_help(void) {
    ...
    vga_puts_color("\n  Milestones (to implement in later stages):\n", VGA_LIGHT_CYAN, VGA_BLACK);
    vga_puts("  ps               – [L09] List processes\n");
    vga_puts("  kill <pid>       – [L09] Terminate a process\n");
    vga_puts("  threads          – [L10] List kernel threads\n");
    vga_puts("  meminfo          – [L11] Show physical memory stats\n");
    vga_puts("  ls / touch / cat – [L12] RAM-disk file system\n\n");
}
...
static void cmd_version(void) {
    ...
    vga_puts("  Architecture : x86 (i686) 32-bit Protected Mode\n");
    vga_puts("  Kernel       : Freestanding C (C99) + NASM\n\n");
}
